...
import database
from auth import is_tool_authorized
...
async def allocate_blood(
    ...
    if inv and inv["blood_type"] == "O-" and inv["units_available"] <= 2:
        ...
        try:
            # Call back into the client session and wait for response
            res = await session.send_request("elicitation/create", elicitation_request)
